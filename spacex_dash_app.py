...
def get_pie_chart(entered_site):
    ...
    if entered_site == 'ALL':
        ...
        filtered_df.rename(columns={'class': 'SuccessRate'}, inplace=True)
        fig = px.pie(
            filtered_df,
            values='SuccessRate',
            names='Launch Site', 
...
def get_scatter_plot (entered_site, entered_payload):
    filtered_df = spacex_df[spacex_df['Payload Mass (kg)'].between(entered_payload[0]-1,entered_payload[1]+1)]
    if entered_site == 'ALL':
        fig = px.scatter(
            filtered_df, 
            x="Payload Mass (kg)",
